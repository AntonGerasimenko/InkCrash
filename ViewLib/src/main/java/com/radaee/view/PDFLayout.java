...
abstract public class PDFLayout
{
	...
	protected float m_zoom_level = 15;//can't be neg value. 15 means 15x(2000%) zooming.
	protected float m_zoom_level_clip = 2.5f;//can't be neg value. 2 means 2x(200%) zooming. when zoom value greater than this, it clip pages as many small bitmaps.
	...
	protected int m_back_color = 0xFFCCCCCC;
	...
	public final void vScrollAbort()
	{
		if(m_doc != null)
		{
			...
			{
				Scroller scroller = new Scroller(m_context);
    ...
    private void vDrawZoom(Canvas canvas, int pageno0, int pageno1, int x, int y)
    {
        m_bmp.eraseColor(m_back_color);
        Canvas bcan = new Canvas(m_bmp);
        while( pageno0 < pageno1 )
        {
            m_pages[pageno0].vDraw(bcan, x, y);
            pageno0++;
        }
        if(Global.dark_mode)
        {
    ...
    private void vDrawNormal(Canvas canvas, int pageno0, int pageno1, int x, int y)
    {
        VPage.VPageRenderResult rets[] = new VPage.VPageRenderResult[pageno1 - pageno0];
        BMP bmp = new BMP();
        bmp.Create(m_bmp);
        while (pageno0 < pageno1)
        {
            rets[pageno0 - m_disp_page1] = m_pages[pageno0].vDraw(m_thread, bmp, x, y);
            pageno0++;
        }
        if(m_cache_use)
        {
            ...
            while (pageno0 < pageno1) {
                m_pages[pageno0].vDrawStep1(bcan, rets[pageno0 - m_disp_page1]);
                pageno0++;
            }
            bmp.Create(m_bmp);
        }
        ...
        while (pageno0 < pageno1)
        {
            m_pages[pageno0].vDrawStep2(bmp, rets[pageno0 - m_disp_page1]);
            pageno0++;
        }
	...
	public void vDraw(Canvas canvas, boolean zooming)
	{
		...
		if(zooming)
		{
            vDrawZoom(canvas, pageno0, pageno1, x, y);
            /*
			Canvas bcan = new Canvas(m_bmp);
			while( pageno0 < pageno1 )
			{
		...
		{
            vDrawNormal(canvas, pageno0, pageno1, x, y);
            /*
			BMP bmp = new BMP();
			bmp.Create(m_bmp);
            while (pageno0 < pageno1)
            {
                m_pages[pageno0].vDraw(m_thread, bmp, x, y);
                pageno0++;
            }
            if (Global.dark_mode) {
                bmp.Invert();
	...
	public final VPage vGetPage(int pageno){return m_pages[pageno];}
     ...
     * Open View
     * @param doc
     * @param cache_path if not null and not empty, it store caches to this dir.
     * @param listener
     */
	public void vOpen(Document doc, String cache_path, LayoutListener listener)
	{
        ...
        boolean cache_dir = (cache_path != null && cache_path.length() > 0);
        if(cache_dir)
        {
            File fdir = new File(cache_path);
            if(!fdir.isDirectory()) cache_dir = false;
        }
        for (int cur = 0; cur < cnt; cur++) {
            m_pages[cur] = new VPage(m_doc, cur, m_bmp_format);
        }
	...
	public boolean vFling(int holdx, int holdy, float dx, float dy, float vx, float vy)
	{
    	...
    	m_scroller.forceFinished(true);
    	m_scroller.fling(vGetX(), vGetY(), (int)(-vx * 2 / 3), (int)(-vy * 2 / 3), -m_w, m_tw, -m_h, m_th);
		return true;
	}
	private int m_pageno = -1;
    protected void vFlushCacheRange()
    {
        float mul = (m_scale + 1) / m_scale_min;
        PDFPos pos1 = vGetPos((int)(-m_w * mul), (int)(-m_h * mul));
        PDFPos pos2 = vGetPos((int) (m_w * mul), (int) (m_h * mul));
        ...
        vpage2.vCacheStart2(m_thread_cache, pos2.x, pos2.y );
        vpage1.vCacheStart0(m_thread_cache, pos1.x, pos1.y);
    }
     ...
     */
	public final int vGetWidth(){return m_w;}

    /**
     * @return height of window
     */
	public final int vGetHeight(){return m_h;}

    /**
     * @return total width of document in view
     */
	public final int vGetTWidth(){return m_tw;}
    /**
     * @return total height of document in view
     */
	public final int vGetTHeight(){return m_th;}
	public final float vGetScale(){return m_scale;}
    @Override
