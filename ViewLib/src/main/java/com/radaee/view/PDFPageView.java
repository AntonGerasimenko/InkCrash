...
public class PDFPageView extends View
{
     ...
     */
    public void vOpen(VThread thread, Document doc, int pageno, int fit_type)
    {
        vClose();
        m_scale = 0;
        m_x = 0;
        ...
        m_thread = thread;
        m_fit_type = fit_type;
        m_vpage = new VPage(m_doc, m_pageno, Bitmap.Config.ARGB_8888);
    }
    ...
    private void SetScale(float scale)
    {
        ...
        SetX(m_x);
        SetY(m_y);
        m_vpage.vLayout(m_thread, m_margin_left, m_margin_top, m_scale, true);
    }
    @Override
    protected void onSizeChanged( int w, int h, int oldw, int oldh )
    {
        ...
        if(m_w > 0 && m_h > 0)
        {
            if(m_doc != null)
            {
                float pw = m_doc.GetPageWidth(m_pageno);
                float ph = m_doc.GetPageHeight(m_pageno);
                float scale1 = (w - m_page_gap) / pw;
                float scale2 = (h - m_page_gap) / ph;
                switch(m_fit_type)
                {
                    case 1:
                        m_scale_min = scale1;
